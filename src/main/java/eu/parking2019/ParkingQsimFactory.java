 ...
 * @author thibautd
 */
public class ParkingQsimFactory implements Provider<Mobsim>{
	...
	public static TransitRouterImpl trImpl;
	...
	public Mobsim get() {
		...
		//QSim qSim = QSimUtils.createDefaultQSim(sc, eventsManager);
		QSim qSim = new QSim(sc, eventsManager);
		
		QNetsimEngineModule.configure(qSim); //Hebenstreit (15.06.2018)
		
		ParkingDepAndMobsimEngine pEngine = new ParkingDepAndMobsimEngine(sc, eventsManager);
		qSim.addMobsimEngine(pEngine );
		qSim.addDepartureHandler(pEngine);
		...
		agentFactory = new ParkingAgentFactory(qSim, pathCalculator, pathCalculatorFactory);
